...
def run_cycle(
    ...
    if state.trade_log or state.active_strategy:
        ...
        coach_result = _call_agent("trading_coach", coach_input, {}, network)
    ...
    builder_result = _call_agent("strategy_builder", builder_input, {}, network)
    ...
    print(f"[Strategy Builder] Generated {len(strategies)} strategies  "
          f"(diversity score: {diversity_score:.1f}/100)")
    ...
    if active_strats:
        ...
        trader_result = _call_agent("trader", trader_input, {}, network)
...
def _call_agent(
    ...
    """
    Send a TASK message directly to a named agent and return its RESULT content.
...
def _print_scorecard(bt_data: dict) -> None:
    ...
    for s in passed:
        ...
        print(
            f"  {marker} {row:<3} {s['name'][:40]:<40} {'PASS':<5}  "
            f"{s['score']:>5.1f}  "
    ...
    for rank_idx, w in enumerate(winners, 1):
        ...
        print(
            f"  {mark} {label}: {w_name}\n"
            f"    Score: {w_score:.1f}/100  |  Return: {w_ret:+.1f}%  |  "
